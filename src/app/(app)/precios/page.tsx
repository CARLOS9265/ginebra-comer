...
export default async function PricesPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-slate-50">Precios internacionales</h1>
      ...
      {recent && recent.length > 0 && (
        <div className="mt-8">
          <h2 className="mb-3 text-sm font-semibold text-slate-200">Últimos días</h2>
          <div className="overflow-x-auto rounded-xl border border-slate-800">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {recent.map((p) => (
                  <tr key={p.price_date} className="hover:bg-slate-900/50">
                    <td className="px-4 py-3 text-slate-300">
                      {new Date(p.price_date + "T00:00:00").toLocaleDateString("es-PE")}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-slate-300">{fmtUSD(p.gold_usd_oz)}</td>
                    <td className="px-4 py-3 text-right font-mono text-slate-300">{fmtUSD(p.silver_usd_oz)}</td>
                    <td className="px-4 py-3 text-right font-mono text-slate-300">{fmtUSD(p.lead_usd_ton)}</td>
