...
import { saveSampleResult, type SampleBatchFormState } from "../actions";

export function SampleResultForm({ batchId }: { batchId: string }) {
  const boundAction = saveSampleResult.bind(null, batchId);
  ...
  return (
    <form action={action} className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="border-t border-dashed border-slate-200 pt-3">
        <p className="mb-2 text-xs font-medium text-slate-500">Ley (para la liquidación de PY)</p>
