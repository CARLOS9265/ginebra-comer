...
export function AddBigBagsSection({ saleLotId, bags }: { saleLotId: string; bags: Bag[] }) {
  ...
  return (
    <form action={action} className="space-y-3 rounded-xl border border-slate-800 bg-slate-900 p-4">
      <div className="max-h-64 overflow-y-auto overflow-x-auto rounded-lg border border-slate-800">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            {bags.map((b) => (
              <tr key={b.id} className="cursor-pointer hover:bg-slate-800/50" onClick={() => toggle(b.id)}>
                ...
                <td className="px-3 py-2 font-mono text-slate-200">{b.code}</td>
                <td className="px-3 py-2 text-slate-300">{b.purchaseLotCode}</td>
